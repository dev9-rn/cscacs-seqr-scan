...
class StudentLoginScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            // username: 'dev7@scube.net.in',
            // password: 'qwertyui',
          
            // username: 'dev12@scube.net.in',
			// password: 'admin@123',
            username:'',
            password:'',
            userNameError: '',
    ...
    forgotPasswordApi = () => {
        ...
        if (!this.state.email_id) {
            ...
            formData.append('institute', this.state.collegeName);
            ...
            fetch(`${URL}registration`, {
                ...
                .then(response => {
                    ...
                    else if (response.status == 404) { utilities.showToastMsg(response.message); }
                    else if (response.status == 422) { utilities.showToastMsg(response.message); }
                    else if (response.status == 400) { utilities.showToastMsg(response.message); }
                    else if (response.status == 403) { utilities.showToastMsg(response.message); this.props.navigation.navigate('StudentLoginScreen') }
                    else if (response.status == 405) { utilities.showToastMsg(response.message); }
    ...
    async callForAPI() {
        ...
        formData.append('institute', this.state.collegeName);
        AsyncStorage.setItem('collegeName', this.state.collegeName)
                                    ...
                                    <Grid style={{ marginTop: 10 }}>
                                        <Col>
                                            <TouchableOpacity style={{ marginTop: 10, paddingLeft: 10 }}>
                                                <Text style={{ color: '#1784C7', fontSize: 13 }} onPress={() => this.props.navigation.navigate('StudentSignupScreen')}>Click here to sign up</Text>
                                            </TouchableOpacity>
                                        </Col>
                                        <Col>
                                            <TouchableOpacity style={{ marginTop: 10, paddingLeft: 10 }}>
...
export default connect(mapStateToProps, mapDispatchToProps)(StudentLoginScreen)